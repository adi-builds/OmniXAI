...
import { useUser, useClerk } from '@clerk/react'  // ✅ add useClerk

const AiTools = () => {
    const navigate = useNavigate()
    const { user } = useUser()          // tells us if user is logged in
    const { openSignIn } = useClerk()   // opens the Clerk login popup

    const handleToolClick = (path) => {
        if (user) {
            navigate(path)    // ✅ logged in → go to the tool
        } else {
            openSignIn()      // ❌ not logged in → show login popup
        }
    }
            ...
            <div className='flex flex-wrap mt-10 justify-center'>
                {AiToolsData.map((tool, index) => (
                    <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:translate-y-1 transition-all duration-300 cursor-pointer'
                        onClick={() => handleToolClick(tool.path)}>  {/* ✅ use handleToolClick */}
