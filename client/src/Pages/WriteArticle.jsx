...
const WriteArticle = () => {
  ...
  const articleLength = [
    { length: 1200, words: '500-800', text: 'Short(500-800 words)' },
    { length: 2000, words: '800-1200', text: 'Medium(800-1200 words)' },
    { length: 3200, words: '1200-1500', text: 'Long(1200+ words)' },
  ]
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Write a detailed, well-structured article about "${input}". The article must be ${selectedLength.words} words long. Use clear headings, useful examples, and complete paragraphs. Do not make it short.`
      const { data } = await axios.post('/api/ai/generate-article', { prompt, length: selectedLength.length }, {  // ✅ fixed: axios.post
    ...
    } catch (error) {  // ✅ fixed: catch is inside try block
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 h-[600px]'>
        ...
        {!content ?(
          ...
          <div className='mt-3 flex-1 min-h-0 overflow-y-auto pr-2 text-sm text-slate-600'>
